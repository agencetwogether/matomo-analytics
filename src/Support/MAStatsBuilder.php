...
class MAStatsBuilder
{
    ...
    public function resolve(): MAStats
    {
        if ($this->getResponse() instanceof MatomoAnalytics) {
            $value = blank($this->type)
                ? $this->getResponse()->trajectoryValue()
                : $this->getResponse()->trajectoryValueAsTimeString();

            return MAStats::make($this->getHeading(), $value)
        ...
        return MAStats::make($this->getHeading(), MatomoAnalytics::for($result)->trajectoryValue())
            ->chart($data)
            ->chartColor('primary')
            ->select($this->getOptions(), $this->getFilter());
    }
