...
class MatomoAnalytics
{
    ...
    public function trajectoryLabel(): string
    {
        return match ($this->getSign()) {
            -1 => __('matomo-analytics::widgets.trending_down'),
            0 => __('matomo-analytics::widgets.steady'),
            1 => __('matomo-analytics::widgets.trending_up'),
            default => __('matomo-analytics::widgets.steady')
        };
    }
    ...
    {
        return match ($this->getSign()) {
            -1 => config('matomo-analytics.trending_down_color'),
            0 => config('matomo-analytics.trending_steady_color'),
            1 => config('matomo-analytics.trending_up_color'),
            default => config('matomo-analytics.trending_steady_color')
        };
    }
    ...
    {
        return match ($this->getSign()) {
            1 => config('matomo-analytics.trending_up_icon'),
            -1 => config('matomo-analytics.trending_down_icon'),
            default => config('matomo-analytics.trending_steady_icon')
        };
    }
